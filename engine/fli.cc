		...
		while (packets < 0) {
			if (packets & 0x4000) {
				y -= packets;
                                ...
                                fli_bitmap_data[(y*fli_bitmap_width)+fli_bitmap_width-1]=(byte)(packets&0xff);
			}
...
static void do_fli_black(byte *p, int sz) {
	...
	V_memset(fli_bitmap_data, 0, fli_bitmap_width*fli_bitmap_height);
	//clear(fli_bitmap);
	...
	while (ret == FLI_OK) {
		...
		if (fli_bmp_dirty_from <= fli_bmp_dirty_to) {
			...
			for (i=fli_bmp_dirty_from; i<=to; i++) {
				V_memcpy(bmp->data+(i*bmp->w), fli_bitmap_data+(i*fli_bitmap_width),
...
static int do_open_fli() {
	...
	fli_bitmap_data=(byte *)valloc(fli_bitmap_width*fli_bitmap_height, "fli_bitmap_data", 0);
	if (!fli_bitmap_data) {
		close_fli();
		return FLI_ERROR;
	}
