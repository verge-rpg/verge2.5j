...
static void Conlib_ConsoleBG()
{
	...
	case 2: {
		...
		pic	=Image_LoadBuf((const char*)Con_GetArg(1));
	// if we couldn't find it, notify user
		if (!pic)
		{
			Console_Printf(va("%s: error opening console graphic", (const char*)Con_GetArg(1)));
			Console_Printf("");
			return;
		}
	...
	// we found it, but it's not the correct size
		...
		{
			vfree(pic);
			Console_Printf(va("%s: invalid console graphic", (const char*)Con_GetArg(1)));
			Console_Printf("");
			return;
		}
...
static void Conlib_PackInfo()
{
	...
	{
	case 2: {
		int mount=atoi((const char*)Con_GetArg(1));
...
static void Conlib_CD_Play()
{
	...
	case 2: {
		/*
		int track=atoi((const char*)Con_GetArg(1));
		CD_Play(track);
		*/
	}
	break;
...
static void Conlib_Vid_Mode()
{
	...
	{
	case 3:
		if (GFX_SetMode( atoi((const char*)Con_GetArg(1)), atoi((const char*)Con_GetArg(2)) ))
		{
			Console_Printf("{||||||||||||}");
			Console_Printf("Loading new video driver...");
			Console_Printf(DriverDesc);
			Console_Printf("{||||||||||||}");
		...
		{
			Console_Printf("Unsupported/unknown video mode.");
		}
		Console_Printf("");
	break;
...
static void Conlib_Cpu_Usage()
{
	...
	case 2:
		cpu_watch=atoi((const char*)Con_GetArg(1)) ? 1 : 0;
	break;
...
static void Conlib_Mount()
{
	...
	{
	case 2:
		MountVFile((const char*)Con_GetArg(1));
		Console_Printf(va("%s mounted.", (const char*)Con_GetArg(1)));
		Console_Printf("");
	break;
...
static void Conlib_Map()
{
	...
	case 2:
		...
		Log(va("MAPswitch=%s", (const char*)startmap));
...
static void Conlib_Ver()
{
	...
	case 1:
#ifdef __WATCOMC__
		extern void Conlib_Ver_Watcom();
		Conlib_Ver_Watcom();
#endif

#ifdef __DJGPP__
		extern void Conlib_Ver_DJGPP();
		Conlib_Ver_DJGPP();
#endif
	break;
...
static void Conlib_Warp()
{
	...
	{
	case 3:
		warpx=atoi((const char*)Con_GetArg(1));
		warpy=atoi((const char*)Con_GetArg(2));
...
static void Conlib_CameraTracking()
{
	...
	case 2:
		cameratracking=(byte)atoi((const char*)Con_GetArg(1));
	break;
...
static void Conlib_RString()
{
	...
	{
	case 1:
		Console_Printf(va("Renderstring: %s", (const char*)rstring));
		Console_Printf("");
	break;
...
static void Conlib_ShowObs()
{
	...
	case 2:
		showobs=atoi((const char*)Con_GetArg(1)) ? 1 : 0;
	break;
...
static void Conlib_Phantom()
{
	...
	case 2:
		phantom=atoi((const char*)Con_GetArg(1)) ? 1 : 0;
	break;
...
static void Conlib_Entity()
{
	...
	{
	case 2: {
		int n=atoi((const char*)Con_GetArg(1));
		if (n<0 || n>=entities)
		{
...
static void Conlib_PlayerSpeed()
{
	...
	case 2:
		player->speed=(byte)atoi((const char*)Con_GetArg(1));
		player->speedct=0;
	break;
...
static void Conlib_SpeedDemon()
{
	...
	case 2:
		speeddemon=atoi((const char*)Con_GetArg(1)) ? 1 : 0;
	break;
...
static void Conlib_SpawnEntity()
{
	...
	case 4: {
		int n=AllocateEntity(
			...
			atoi((const char*)Con_GetArg(1)), atoi((const char*)Con_GetArg(2)),
			(const char*)Con_GetArg(3));
...
static void Conlib_ShowZones()
{
	...
	case 2:
		showzone=atoi((const char*)Con_GetArg(1)) ? 1 : 0;
	break;
