	...
	: public linked_node
{
	...
	message_t(string_t s, int expire)
...
static char runprf[3];

// -- final numbers --

static int fps=0;
static char profile[3];
...
void RenderGUI()
{
	...
	x=screen_width -Font_GetWidth(0)*10 -4;
	y=screen_length -Font_GetLength(0)*4 -4;
...
void Message_Send(string_t text, int duration)
{
	Log(va("Message: %s", (const char*)text));
