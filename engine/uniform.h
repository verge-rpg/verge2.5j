...
// UNIROW: data-type used for referencing a row within a unitrk stream.
//         This is not used internally.  It is used to help optimize the
//         player references into the unitrack structures.

typedef struct UNIROW
{   UBYTE   rowrep;   // the repeat count position within the row.
    UBYTE   *row;     // pointer to the actual row.
} UNIROW;
...
{   INSTRUMENT  *i;
    ...
    UBYTE  dca;           // duplicate check action
    UBYTE  dct;           // duplicate check type
    UNIROW row;           // row currently playing on this channel
    ...
    UBYTE  s3mtempo;      // s3m tempo set / slide memory
    ...
    UBYTE  ssmem_notecut; // last used SCx data
    UBYTE  ssmem_notedelay;// last used SDx Data...
...
// A special WinAMP addition: PMSTATE (Player Mixer).
// 256 of these are built for the module when loaded.  This allows the user to skip
// to any portion of the song and have replay be 100% perfect!  A pioneer among players!

typedef struct PMSTATE
{   MP_VOICE    *voice;        // Save states of voice and control player stuff
    MP_CONTROL  *control;      //  (used for smoothskip, ignored for quicklookup)

    // save state of the module-wide variables
    
    long        curtime;        // current time in the song (to 1/10000 of a second).
    UBYTE       bpm;            // current beats-per-minute speed
    UWORD       sngspd;         // current song speed
    SWORD       volume;
    UWORD       numrow, vbtick, patpos;
    SWORD       sngpos;
    ...
    UBYTE      *pos_played;    // Need this else songs will repeat when they shouldn't.
    void        *virtch;       // save state of virtch (smoothskip only)
} PMSTATE;
...
typedef struct UNIMOD
{
    ...
    int         numchn;         // number of module channels
    int         numvoices;      // max # voices used for full NNA playback
    ...
    SWORD       panning[64];    // 64 panning positions
    ...
    UBYTE       globalslide;    // global volume slide rate
    //UBYTE       patloop;       // time-to-loop flag set by E6x PT Effect
    UWORD       patbrk;         // position where to start a new pattern
    UBYTE       patdly;         // patterndelay counter (command memory)
    UBYTE       patdly2;        // patterndelay counter (real one)
    SWORD       posjmp;         // flag to indicate a position jump is needed...
...
extern void    UniSetRow(UNIROW *r);
extern UBYTE   UniGetByte(void);
extern void    UniFindRow(UNIROW *r, UBYTE *t,UWORD row);
extern void    UniNextRow(UNIROW *r);
...
extern UNIMOD  *MikMod_LoadSongFP(FILE *fp, int maxchan);
extern UNIMOD  *MikMod_LoadSong(CHAR *filename, int maxchan);
extern UNIMOD  *MikMod_LoadSongInfo(CHAR *filename);
extern void     MikMod_FreeSong(UNIMOD *mf);
...
extern BOOL    ReadComment(UWORD len);
extern BOOL    AllocPositions(int total);
extern BOOL    AllocPatterns(void);
extern BOOL    AllocTracks(void);
extern BOOL    AllocInstruments(void);
extern BOOL    AllocSamples(BOOL ext);
...
extern void   Player_SetPosition(int pos);
...
extern void   Player_SetPosTime(long time);
...
extern void   MP_SetPosition(UNIMOD *mf, int pos);
...
extern void MP_Cleaner(UNIMOD *mf);
extern BOOL MP_BuildLookups(UNIMOD *mf);
