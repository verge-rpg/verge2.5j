...
//char *stringbuf;
string_t*	vc_strings		=0;		// vc string workspace
int		stralloc		=0;

int		vcreturn		=0;		// return value of last int function
string_t	vcretstr	="";	// return value of last string function
...
static string_t str_stack[1024 +20];
...
static void PushStr(string_t s)
{
...
#ifdef DEBUG_LOCALS
	Log(va("base=%-04d, push$ %-60s", str_stack_base, (const char*)str_stack[str_stack_ptr]));
#endif
	str_stack_ptr++;
}
static string_t PopStr()
{
	if (str_stack_ptr<=0 || str_stack_ptr>1024)
	...
	--str_stack_ptr;
#ifdef DEBUG_LOCALS
	Log(va("base=%-04d, pop$  %-60s", str_stack_base, (const char*)str_stack[str_stack_ptr]));
#endif
...
// allocate global integer and string arrays
	...
	if (stralloc)
	{
		vc_strings=new string_t[stralloc]; //(string *)valloc(sizeof(string) * stralloc, "vc_strings", OID_VC);
	}
...
void  ReadVCVar()
{
	Console_Printf("ReadVCVar disabled.");

	/*
  int i=0;
  int j=0;

  string_t arg1=Con_GetArg(1).lower();

  for (i=0; i<=numvars; i++)
    if (!strcmp(vars[i].vname, (const char*)arg1))
		break;
  ...
  if (i<numvars)
  {
    ...
    if (vars[i].arraylen>1)
      j+=V_atoi((const char*)Con_GetArg(2));
    j=globalint[j];
...
void WriteVCVar()
{
	Console_Printf("WriteVCVar disabled.");
  ...
  if (i<numvars)
  {
    ...
    {
      j+=V_atoi((const char*)Con_GetArg(2));
      globalint[j]=V_atoi((const char*)Con_GetArg(3));
    }
    else globalint[j]=V_atoi((const char*)Con_GetArg(2));
    sprintf(strbuf,"%s:%d", vars[i].vname, V_atoi((const char*)Con_GetArg(2)));
    Console_Printf(strbuf);
    return;
  }
  for (i=0; i<=numstr; i++)
...
void ExecuteVC()
{
	...
	{
		if (kill) break;
		if (key_down[K_ALT] && key_down['x']) Sys_Error("");
