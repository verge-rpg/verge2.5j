...
// CHANGELOG:
// <aen, apr 21>
// + changed err() to take variable number of args.
// + removed errf() and errn()
// <aen, apr 22>
// + hehe, silly bug in err()
// -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
...
// <aen, apr 21>
// - only used in djgpp mode13h.c, vesa.c, & modex.c ?
int sgn(int x)
{
  if (x>=0) return 1;
  return 0;
...
{ char *tmp;
  ...
  if (!tmp) err("Failed allocating %d bytes.", amount);
  memset(tmp, 0, amount);
  return tmp;
}
*/
